 ...
 * @Last Modified time: 2024-06-08 22:57:56
 */
  ...
  for (const Layer &layer : layers) {
    if (layer.visible) {
      for (const auto &tilePair : layer.tiles) {
        // Find tile data for tile with required tileid from tileset. Use tile's
        // Draw()
...
const Vector2 Map::GetSpawnpoint() {
  // Find tile spawnpoint tile ids
  std::vector<int> spawnpointIds = tileset.GetSpawnpointTileIds();

  // Find tile with spawnpoint tile id and return its position
  auto spawnpointTile = GetFirstTileWithId(spawnpointIds[0]);
  if (spawnpointTile == nullptr) {
    std::cout << "Spawnpoint not found, spawning at 0,0\n";
    return Vector2{0, 0};
  }
  return spawnpointTile->second;
}

const std::pair<int, Vector2> *Map::GetFirstTileWithId(int id) {
  for (const auto &layer : layers) {
    for (const std::pair<int, Vector2> &tile : layer.tiles) {
      if (tile.first == id)
        return &tile;
    }
  }
